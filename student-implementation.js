...
function handleKeyPress(key) {
    ...
    // HINT: Call submitGuess() if complete, show error message if not
        if (isGuessComplete()) {
            ...
            showMessage("Error: Incomplete Guess", 'error');
        }
    ...
    if (isGuessComplete()) {
        if (WordleWords.isValidWord(currentGuess)) {
            for (let index = 0; index < currentGuess.length; index++) {
                results.push(checkLetter(currentGuess[index], index, currentWord));
            }
            console.log("Results: " + results);
            console.log("Correct word: " + currentWord);
            for (let index = 0; index < results.length; index++) {
                let tile = getTile(currentRow, index);
                setTileState(tile, results[index]);
            }
            numGuesses++;
            updateKeyboardColors(currentGuess, results);
            processRowReveal(currentRow, results);
            updateGameState(currentGuess.toUpperCase() === currentWord);
        } else {
            showMessage("Error: Guess is not a real word", 'error');
            shakeRow(currentRow);
        }
    }
...
function checkLetter(guessLetter, position, targetWord) {
    ...
    console.log('Checking letter:', guessLetter, 'at position:', position);
...
function showEndGameModal(won, targetWord) {
    ...
    showModal(won, targetWord, numGuesses);
